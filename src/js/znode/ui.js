$(function(){
  ...
  function contestMenuHandler(key, opt) {
    if("newNode" == key) {
      graph.addNodeAtMouse();
    } else if("toProdJson" == key) {
  ...
  $.contextMenu({
    ...
    items: {
        newNode: {name: "New Node", callback: contestMenuHandler },
        toProdJson: {name: "To Production Json", callback: contestMenuHandler }
    }
